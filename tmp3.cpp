...
Coro<DataType> getNext(int start = 0, int step = 1) {
    auto value = start;
    while (true) {
        co_yield value;
        value += step;
...
TEST_CASE("PathSpace") {
    ...
    SUBCASE("Insert And Grab") {
        // Insert
        CHECK(space.nbrSpaces()==0);
        CHECK(space.insert("/test", 5));
        CHECK(space.nbrSpaces()==1);
        CHECK(space.insert("/test", 4));
        CHECK(space.nbrSpaces()==2);
        CHECK(space.insert("/test", 3));
        CHECK(space.nbrSpaces()==3);
    ...
    SUBCASE("View Linking") {
        ...
        view.insert("/test", 5);
        CHECK(space->nbrSpaces() == view.nbrSpaces());
    ...
    SUBCASE("Sub Paths") {
        ...
        CHECK(parent.value().nbrSpaces()==1);
    ...
    SUBCASE("Coroutine") {
        PathSpaceTE space = PathSpace{};
        space.insert("coro", []()->Coro<DataType>{co_yield 345;});
    }
}