...
inline auto Codex2::grab(Path const &range, void *obj, std::type_info const *type, std::size_t const size) -> bool {
    if(range.isAtSpace()) {
        if(this->scrolls.count(&typeid(PathSpace2)))
            return this->scrolls.at(&typeid(PathSpace2)).grab(range, obj, type, size);
    } else {
        if(type==&typeid(std::string) && this->scrolls.count(&typeid(char*)))
            return this->scrolls.at(&typeid(char*)).grab(range, obj, type, size);
        if(this->scrolls.count(type))
            return this->scrolls.at(type).grab(range, obj, type, size);
    }
    return false;
}