...
struct MutexLog {
    ...
    auto lock()          {LOG_M("Starting lock for {} {}",          this->name, fmt::ptr(this))this->mutex.lock();         this->status = 2;LOG_M("Lock finished, status for {} {} {}",          this->name, this->status, fmt::ptr(this))}
    auto unlock()        {LOG_M("Starting unlock for {} {}",        this->name, fmt::ptr(this))this->mutex.unlock();       this->status = 0;LOG_M("Unlock finished, status for {} {} {}",        this->name, this->status, fmt::ptr(this))}
    auto lock_shared()   {LOG_M("Starting lock_shared for {} {}",   this->name, fmt::ptr(this))this->mutex.lock_shared();  this->status = 1;LOG_M("Lock_shared finished, status for {} {} {}",   this->name, this->status, fmt::ptr(this))}
    auto unlock_shared() {LOG_M("Starting unlock_shared for {} {}", this->name, fmt::ptr(this))this->mutex.unlock_shared();this->status = 0;LOG_M("Unlock_shared finished, status for {} {} {}", this->name, this->status, fmt::ptr(this))}
private:
    T mutex;
